...
void RobotGrid::draw_grid_lines()
{
    ...
    // loddrett
    for (int x = x_pos; x < w_size; x += cell_width){
        
        Point startLine{x, y_pos};
        Point endLine{x, h_size};
        window.draw_line(startLine, endLine);
        //rows++;
    }
    // missing lower line
    window.draw_line({x_pos, h_size}, {w_size, h_size});

    // vannrett
    for (int y = y_pos; y < h_size; y += cell_height){
        Point startLine{x_pos, y};
        Point endLine{w_size, y};
        window.draw_line(startLine, endLine);
        //cols++;
    }
    // missing right line
    window.draw_line({w_size, h_size}, {w_size, y_pos});
...
Point RobotGrid::get_grid_cell_center_coord(int x, int y) const
{
  ...
  // upper left corner
  int x_upper_left = x_pos + (x * cell_width);
  int y_upper_left = y_pos + (y * cell_height);
  
  int x_offset = cell_width/2;
  int y_offset = cell_height/2;

  Point center{x_upper_left + x_offset, y_upper_left + y_offset};
  return center; 
...
void RobotGrid::draw_robots()
{
  ...
  // tar utgangspunkt i at grid alltid er kvadratisk 
  int radius = cell_width/2;

  for (auto it = robots.begin(); it != robots.end(); it++){
    window.draw_circle(get_grid_cell_center_coord(it->second->pos),
        radius, it->second->color);
...
void RobotGrid::delete_robot(string name)
{
  ...
  // uniqe pointer will automatily fix dynamic memory
  robots.erase(name);
...
void RobotGrid::rename_robot(string name, string new_name)
{
  ...
  // also update key in map, creates new element and delte old
...
void RobotGrid::check_coord_bounds(Point p) const
{
  ...
  // check x
  if (p.x < 0){
    throw string("x < 0, not valid");
  } else if (p.x >= cols){
    throw string("x > columns, not valid");
  }

  // check y
  if (p.y < 0 ){
    throw string("y < 0, not valid");
  } else if (p.y >= rows){
    throw string("y > rows, not valid");
  }
...
void RobotGrid::check_name_available(string name) const
{
  ...
  try {
    ...
    // this cathes if key do not exists
    nameValid = true;
  }

  if (!nameValid){
    throw string(name + "allready exists");
  }
...
void RobotGrid::check_name_exists(string name) const
{
  ...
  try { // exsists
    ...
    throw string(name + " do not exist");
  }
...
void RobotGrid::check_coord_empty(Point p, string name, bool is_moving) const
{
  ...
  for (auto it = robots.begin(); it != robots.end(); it++){
    // same position
    if (it->second->pos.x == p.x & it->second->pos.y == p.y){
      // is not moving
      if (!is_moving){
        throw string("there is another robot at your requested position");
        return;
      }
      // is moving, and the robot at req pos is not yourself 
      else if (it->first != name){
        throw string("there is antoher robot at your requested position");
      }
